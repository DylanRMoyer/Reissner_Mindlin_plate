...
def augment_stiffness_matrix(K, stiffness, phi, global_dofs_parent):
    ...
    vals = stiffness * np.outer(phi_ext[nz], phi_ext[nz]).flatten()
...
def validate_augmented_system(
    ...
    assert(np.isclose(K_aug[n, n], vamm.stiffness, rtol=1e-9)), \
           f"K_aug[n,n] = {K_aug[n, n]}, expected: {vamm.stiffness}"
    assert(np.isclose(M_aug[n, n], vamm.mass, rtol=1e-9)), \
           f"M_aug[n,n] = {M_aug[n, n]} expected: {vamm.mass}"
    ...
    for k, dof_i in enumerate(global_dofs_parent):
        assert(np.isclose(K_aug[dof_i, dof_i], K_sp[dof_i, dof_i] + vamm.stiffness * phi[k]**2, rtol = 1e-9)), \
            f"K_aug[i0, i0] = {K_aug[dof_i, dof_i]} expected: {K_sp[dof_i, dof_i] + vamm.stiffness * phi[k]**2}"
    ...
    assert np.isclose(row_n[n], vamm.mass, rtol = 1e-9),\
        f"M_aug[n, n] = {M_aug[n, n]}, expected: {vamm.mass}"
