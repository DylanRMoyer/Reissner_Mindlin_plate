...
def solve_evp(
        domain, function_space, problem, bcs, vamm = None,
    ...
    if vamm is not None:
        ...
        K_aug, n = augment_stiffness_matrix(K, vamm.stiffness, phi, global_dofs_parent)
        M_aug = augment_mass_matrix(M, vamm.mass)

        # Validate augmentation
        if validate_augmentation:
            validate_augmented_system(K_aug, M_aug, K, n, vamm, phi, global_dofs_parent)
    ...
    for i in range(eps.getConverged()):
        ...
        q_r_value = vr.getArray()[n].real if vamm is not None else None
