...
# $\textsf{F} = \kappa G h$ with a shear correction
# factor $\kappa = 5/6$ for a homogeneous plate of thickness $h$:
...
# constant value of 1 in the thin plate. This thin plate limit will be used to check the sensitivity of the element to
# shear locking. Formulations which exhibit shear locking are expected to provide overly stiff results
# (low values of the deflection) for a given mesh in the limit of small thickness $h \to 0$.

f = -D / 1.265319087e-3  # with this we have w_Love-Kirchhoff = 1.0

# Continuous interpolation using degree $d=\texttt{deg}$ is chosen for both deflection and rotation.
# Note that when using quadrilaterals, we have different choices of interpolation over the cell. We will not discuss
# here variants such as `equispaced` or `GLL` variants which mostly differ in the dof location up to $d=3$,
# see [here for more details](https://defelement.com/elements/lagrange.html). However, already for $d=2$,
# we may have the choice of using an 8 or a 9-dof quadrilateral.
#
# `````{grid}
# :gutter: 3
#
...
# ## Results
#
# We provide here some results for $h=0.001$ using either $N=10$ or $N=50$ quads per side and for different types of
...
#   - 1.00002
# ```
# The results show that the low-order elements $d=1$ always lock very strongly but accurate estimates are obtained using selective reduced integration (SRI).
# The Serendipity element `S` exhibits a notable locking behavior which is not necessarily fixed using SRI. On the contrary, for `Q` elements,
# locking is less pronounced and the behavior of the element is improved when using SRI.
